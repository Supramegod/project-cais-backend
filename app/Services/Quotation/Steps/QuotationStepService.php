...
class QuotationStepService
{
    ...
    public function __call(string $method, array $args): void
    {
        if (preg_match('/^updateStep(\d+)$/', $method, $m)) {
            $step = (int) $m[1];
            if ($step === 4) {
                app(\App\Services\Quotation\Domain\CostingService::class)->execute($args[0], $args[1] ?? null);
                return;
            }
            if ($step === 11) {
                app(\App\Services\Quotation\Domain\PricingService::class)->execute($args[0], $args[1] ?? null);
                return;
            }
            if ($step === 12) {
                app(\App\Services\Quotation\Domain\FinalizationService::class)->execute($args[0], $args[1] ?? null);
                return;
            }
