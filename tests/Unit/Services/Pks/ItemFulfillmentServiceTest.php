...
class ItemFulfillmentServiceTest extends TestCase
{
    ...
    public function test_create_fulfillment_produces_log_entry(): void
    {
        ...
        $this->assertDatabaseHas('sl_pks_item_fulfillment_log', [
            'aksi' => 'create',
            'qty_sesi_ini' => 5,
            'remaining_sebelum' => 10,
            'remaining_sesudah' => 5,
        ]);

        $logCount = PksItemFulfillmentLog::count();
    ...
    public function test_edit_fulfillment_produces_audit_log(): void
    {
        ...
        $this->assertDatabaseHas('sl_pks_item_fulfillment_log', [
            'aksi' => 'edit',
            'qty_sesi_ini' => 3, // delta: 8 - 5
            'fulfillment_id' => $fulfillment->id,
        ]);

        $logCount = PksItemFulfillmentLog::where('fulfillment_id', $fulfillment->id)->count();
